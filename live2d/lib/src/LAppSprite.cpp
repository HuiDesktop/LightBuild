...
LAppSprite::LAppSprite(float x, float y, float width, float height, unsigned int textureId, unsigned int programId)
    : _rect()
{
    ...
    // 何番目のattribute変数か
    _positionLocation = rlGetLocationAttrib(programId, "position");
    _uvLocation      = rlGetLocationAttrib(programId, "uv");
    _textureLocation = rlGetLocationAttrib(programId, "texture");
    _colorLocation = rlGetLocationAttrib(programId, "baseColor");
...
void LAppSprite::RenderImmidiate(unsigned int textureId, const float uvVertex[8]) const
{
    // 画面サイズを取得する
    //int maxWidth, maxHeight;
    //glfwGetWindowSize(LAppDelegate::GetInstance()->GetWindow(), &maxWidth, &maxHeight);
