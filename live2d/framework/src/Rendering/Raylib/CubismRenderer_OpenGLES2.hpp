...
private:
     ...
     */
    void SetupClippingContext(CubismModel& model, CubismRenderer_OpenGLES2* renderer, int lastFBO, int lastViewport[4]);
...
private:
    ...
    */
    ...
    {
        unsigned int ShaderProgram;               ///< シェーダプログラムのアドレス
        ...
        int UniformMatrixLocation;        ///< シェーダプログラムに渡す変数のアドレス(Matrix)
        int UniformClipMatrixLocation;    ///< シェーダプログラムに渡す変数のアドレス(ClipMatrix)
        int SamplerTexture0Location;      ///< シェーダプログラムに渡す変数のアドレス(Texture0)
        int SamplerTexture1Location;      ///< シェーダプログラムに渡す変数のアドレス(Texture1)
        int UniformBaseColorLocation;     ///< シェーダプログラムに渡す変数のアドレス(BaseColor)
        int UniformMultiplyColorLocation; ///< シェーダプログラムに渡す変数のアドレス(MultiplyColor)
        int UniformScreenColorLocation;   ///< シェーダプログラムに渡す変数のアドレス(ScreenColor)
        int UnifromChannelFlagLocation;   ///< シェーダプログラムに渡す変数のアドレス(ChannelFlag)
    };
    ...
    /**
     * @brief   シェーダプログラムの一連のセットアップを実行する TODO: Disable VBO after draw!
     *
     * @param[in]   renderer              ->  レンダラのインスタンス
     ...
     */
    void SetupShaderProgram(CubismRenderer_OpenGLES2* renderer, unsigned int textureId
     ...
     * @retval      false           ->  リンク失敗
     */
    // csmBool LinkProgram(unsigned int shaderProgram);
     ...
     * @retval      false           ->  異常
     */
    // csmBool ValidateProgram(unsigned int shaderProgram);
...
private:
     ...
     */
    void SetGlEnable(unsigned int index, unsigned char enabled);
     ...
     * @param[in]   enabled ->  trueなら有効にする
     */
    void SetGlEnableVertexAttribArray(unsigned int index, int enabled);

    int _lastArrayBufferBinding;          ///< モデル描画直前の頂点バッファ
    int _lastElementArrayBufferBinding;   ///< モデル描画直前のElementバッファ
    int _lastProgram;                     ///< モデル描画直前のシェーダプログラムバッファ
    int _lastActiveTexture;               ///< モデル描画直前のアクティブなテクスチャ
    int _lastTexture0Binding2D;           ///< モデル描画直前のテクスチャユニット0
    int _lastTexture1Binding2D;           ///< モデル描画直前のテクスチャユニット1
    int _lastVertexAttribArrayEnabled[4]; ///< モデル描画直前のテクスチャユニット1
    unsigned char _lastScissorTest;             ///< モデル描画直前のGL_VERTEX_ATTRIB_ARRAY_ENABLEDパラメータ
    unsigned char _lastBlend;                   ///< モデル描画直前のGL_SCISSOR_TESTパラメータ
    unsigned char _lastStencilTest;             ///< モデル描画直前のGL_STENCIL_TESTパラメータ
    unsigned char _lastDepthTest;               ///< モデル描画直前のGL_DEPTH_TESTパラメータ
    unsigned char _lastCullFace;                ///< モデル描画直前のGL_CULL_FACEパラメータ
    int _lastFrontFace;                   ///< モデル描画直前のGL_CULL_FACEパラメータ
    unsigned char _lastColorMask[4];            ///< モデル描画直前のGL_COLOR_WRITEMASKパラメータ
    int _lastBlending[4];                 ///< モデル描画直前のカラーブレンディングパラメータ
    int _lastFBO;                         ///< モデル描画直前のフレームバッファ
    int _lastViewport[4];                 ///< モデル描画直前のビューポート
};
...
public:
     ...
     *
     */
    void BindTexture(csmUint32 modelTextureNo, unsigned int glTextureNo);
     ...
     */
    const csmMap<csmInt32, unsigned int>& GetBindedTextures() const;
    ...
    void SetViewport(int w, int h) {
        _rendererProfile._lastViewport[0] = 0;
        _rendererProfile._lastViewport[1] = 0;
        _rendererProfile._lastViewport[2] = w;
        _rendererProfile._lastViewport[3] = h;
    }
...
private:
    ...
    csmMap<csmInt32, unsigned int>            _textures;                      ///< モデルが参照するテクスチャとレンダラでバインドしているテクスチャとのマップ
